...
import { admins as seed, type Admin } from "@/lib/super-admin-data";

export const Route = createFileRoute("/manage-admin")({
  head: () => ({
    meta: [
      { title: "Manage Business Admins — Super Admin" },
      { name: "description", content: "View, edit, activate or deactivate business admin accounts." },
      { property: "og:title", content: "Manage Business Admins — Super Admin" },
      { property: "og:description", content: "Business admin accounts across every tenant." },
    ],
  }),
      ...
      render: (r) => (
        <div className="flex flex-wrap gap-1.5">
          <Button asChild size="sm" variant="outline" className="h-7 rounded-md px-2 text-xs">
            <Link to="/edit-admin" search={{ id: r.id }}>Edit</Link>
          </Button>
