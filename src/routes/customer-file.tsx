...
import { currency, customer, customerTransactions } from "@/lib/sales-ops-data";

export const Route = createFileRoute("/customer-file")({
  head: () => ({
    meta: [
      { title: "Customer File — Meridian POS" },
      { name: "description", content: "View a customer's account balance, credit history and recent transactions." },
      { property: "og:title", content: "Customer File — Meridian POS" },
      { property: "og:description", content: "Customer account balance and transaction history." },
    ],
  }),
  component: CustomerFilePage,
});

type Tx = (typeof customerTransactions)[number];
