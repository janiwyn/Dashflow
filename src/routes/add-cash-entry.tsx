...
import { cashBookEntries as seedEntries } from "@/lib/accounting-data";

export const Route = createFileRoute("/add-cash-entry")({
  head: () => ({
    meta: [
      { title: "Add Cash Entry — Meridian POS" },
      { name: "description", content: "Record a manual cash or bank receipt/payment entry." },
      { property: "og:title", content: "Add Cash Entry — Meridian POS" },
      { property: "og:description", content: "Record a manual cash or bank receipt/payment entry." },
    ],
  }),
  component: AddCashEntryPage,
});

type Entry = { date: string; particulars: string; cashIn: number; bankIn: number; cashOut: number; bankOut: number; source: string };

function AddCashEntryPage() {
