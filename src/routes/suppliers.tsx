import { createFileRoute } from "@tanstack/react-router";

import { AppShell } from "@/components/app-shell";
import { DataTable, type Column } from "@/components/data-table";
import { Button } from "@/components/ui/button";
import { currency, suppliers } from "@/lib/pos-data";

export const Route = createFileRoute("/suppliers")({
  head: () => ({
    meta: [
      { title: "Suppliers — Meridian POS" },
      { name: "description", content: "Supplier accounts, delivery history and payables in one register." },
      { property: "og:title", content: "Suppliers — Meridian POS" },
      { property: "og:description", content: "Supplier accounts, deliveries and payables." },
    ],
  }),
  component: SuppliersPage,
});

type Supplier = (typeof suppliers)[number];

const columns: Column<Supplier>[] = [
  { key: "name", header: "Supplier", render: (s) => <span className="font-medium">{s.name}</span> },
  { key: "category", header: "Category", render: (s) => <span className="text-muted-foreground">{s.category}</span> },
  { key: "last", header: "Last delivery", render: (s) => <span className="text-muted-foreground">{s.lastDelivery}</span> },
  {
    key: "payable",
    header: "Payable",
    align: "right",
    render: (s) => (
      <span className={`num font-semibold ${s.payable > 0 ? "text-destructive" : "text-muted-foreground"}`}>
        {currency(s.payable)}
      </span>
    ),
  },
];

function SuppliersPage() {
  return (
    <AppShell
      title="Suppliers"
      subtitle={`${suppliers.length} active suppliers · ${currency(257700)} payable`}
      actions={
        <Button size="sm" className="rounded-lg">
          New purchase order
        </Button>
      }
    >
      <DataTable title="Supplier register" columns={columns} rows={suppliers} minWidth={640} />
    </AppShell>
  );
}
