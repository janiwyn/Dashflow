...
import { currency, branches } from "@/lib/pos-data";
import { pettyCashBalanceActions, pettyCashTransactions } from "@/lib/accounting-data";

export const Route = createFileRoute("/petty-cash")({
  head: () => ({
    meta: [
      { title: "Petty Cash — Meridian POS" },
      { name: "description", content: "Manage the petty cash float and record small transactions." },
      { property: "og:title", content: "Petty Cash — Meridian POS" },
      { property: "og:description", content: "Manage the petty cash float and record small transactions." },
    ],
  }),
  component: PettyCashPage,
});

type Action = (typeof pettyCashBalanceActions)[number];
type Txn = (typeof pettyCashTransactions)[number];
