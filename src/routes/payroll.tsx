...
import { employees, payrollRecords as initialRecords, type PayrollRecord } from "@/lib/hr-data";

export const Route = createFileRoute("/payroll")({
  head: () => ({
    meta: [
      { title: "Payroll — Meridian POS" },
      { name: "description", content: "Process employee payroll, allowances, deductions and payment status." },
      { property: "og:title", content: "Payroll — Meridian POS" },
      { property: "og:description", content: "Process employee payroll, allowances, deductions and payment status." },
    ],
  }),
      ...
      render: (r) => (
        <div className="flex justify-end gap-2">
          ...
          <Button asChild size="sm" variant="secondary" className="rounded-lg">
            <Link to="/payslip" search={{ id: r.id }}><FileText className="size-3.5" /> Payslip</Link>
          </Button>
        </div>
      ),
    },
