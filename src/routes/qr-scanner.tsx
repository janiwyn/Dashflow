...
import { currency, remoteOrders } from "@/lib/sales-ops-data";

export const Route = createFileRoute("/qr-scanner")({
  head: () => ({
    meta: [
      { title: "QR Scanner — Meridian POS" },
      { name: "description", content: "Scan a customer's order QR code to pull up their remote order and record the sale." },
      { property: "og:title", content: "QR Scanner — Meridian POS" },
      { property: "og:description", content: "Scan and complete remote pickup orders." },
    ],
  }),
...
function QrScannerPage() {
  const [scanned, setScanned] = useState<(typeof remoteOrders)[number] | null>(null);
