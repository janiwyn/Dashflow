...
import { businesses as seed, type Business } from "@/lib/super-admin-data";

export const Route = createFileRoute("/manage-business")({
  head: () => ({
    meta: [
      { title: "Manage Businesses — Super Admin" },
      { name: "description", content: "View, edit, suspend or activate every business registered on the platform." },
      { property: "og:title", content: "Manage Businesses — Super Admin" },
      { property: "og:description", content: "Every tenant business, admin contact and subscription status." },
    ],
  }),
      ...
      render: (r) => (
        <div className="flex flex-wrap gap-1.5">
          <Button asChild size="sm" variant="outline" className="h-7 rounded-md px-2 text-xs">
            <Link to="/view-business" search={{ id: r.id }}>View</Link>
          </Button>
          <Button asChild size="sm" variant="outline" className="h-7 rounded-md px-2 text-xs">
            <Link to="/edit-business" search={{ id: r.id }}>Edit</Link>
          </Button>
