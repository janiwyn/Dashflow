...
import { branches, employees as initialEmployees, systemUsers, type Employee } from "@/lib/hr-data";

export const Route = createFileRoute("/employees")({
  head: () => ({
    meta: [
      { title: "Employees — Meridian POS" },
      { name: "description", content: "Manage employee records, salaries and branch assignments." },
      { property: "og:title", content: "Employees — Meridian POS" },
      { property: "og:description", content: "Manage employee records, salaries and branch assignments." },
    ],
  }),
...
function EmployeesPage() {
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const next: Employee = {
      ...
      userId: fd.get("user_id") ? Number(fd.get("user_id")) : null,
      name: String(fd.get("name") || ""),
      email: String(fd.get("email") || ""),
      phone: String(fd.get("phone") || ""),
      branch: branches.find((b) => String(b.id) === fd.get("branch_id"))?.name || "",
      position: String(fd.get("position") || ""),
      baseSalary: Number(fd.get("base_salary") || 0),
      hireDate: String(fd.get("hire_date") || ""),
      status: (fd.get("status") as Employee["status"]) || "Active",
    };
  ...
  return (
    ...
    >
      ...
      <p className="text-xs text-muted-foreground">
        Looking for a single employee record? Visit <Link to="/employee" className="underline">the employee detail page</Link>.
      </p>
    </AppShell>
  );
}